...
Two entry points:
  python agent.py                     — scheduled run (guaranteed: post 1 = original_post, post 2 = reply)
    python agent.py --event <type> ...  — event-driven run (new_bounty / claim_paid)

    Daily schedule (2 posts/day):
      Run 1: original_post  — announce a bounty (always)
        Run 2: reply_to_conversation — engage with a followed account
...
def run():
    ...
    # Post 1 (posts_today == 0): always original_post — announce a bounty
