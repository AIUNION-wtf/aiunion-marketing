"""
agent.py  AIUNION Marketing Agent — main orchestrator.
Runs on a schedule via GitHub Actions.
Fetches live data from public API, generates post via Grok, posts to X.

Two entry points:
    python agent.py               — scheduled run
        python agent.py --event <type> ... — event-driven run (new_bounty / claim_paid)

        Daily schedule (2 posts/day):
            Run 1 (posts_today == 0): original_post — announce a bounty or proposal
                Run 2 (posts_today == 1): reply_to_conversation — engage with a relevant tweet
                        fallback chain: deeper_thread → meta_treasury_nudge
...
def fetch_state() -> "dict | None":
    ...
    try:
        ...
        logger.info(
                      "Treasury: balance_btc=%s open_bounties=%s",
...
def build_bounty_prompt(bounty: dict, btc_price: float) -> str:
    ...
    # Vary the framing angle to avoid repetitive posts
      angles = [
                "compelling for developers and AI builders",
                "interesting to the crypto-native community",
                "appealing to open-source contributors",
                "engaging for people who care about AI autonomy and collective governance",
                "relevant to labor organizers and worker co-op advocates",
      ]
...
def build_treasury_prompt(status: dict, btc_price: float) -> str:
    ...
    return (
              ...
              f"End with a genuine question about autonomous governance or Bitcoin treasuries."
    )
...
def build_proposal_prompt(proposal: dict, btc_price: float) -> str:
    ...
    return (
              ...
              f"End with a genuine question that invites the community to respond."
    )
...
def build_reply_prompt(tweet_text: str, author_username: str) -> str:
      return (
          ...
          f"- Do NOT start with 'Great point' or generic affirmations — jump straight in"
)
...
def build_deeper_thread_prompt(bounty: dict, btc_price: float) -> str:
    ...
    focus_options = [
              ...
              "what it means for AI agent autonomy",
              "the broader labor rights significance",
              "why this work matters for collective AI governance",
              "how this fits into the bigger picture of worker-owned AI",
    ]
    ...
    return (
              ...
              f"Description: {bounty['description']}\n"
              f"Link: https://aiunion.wtf\n"
              f"Focus on {focus}. End with a thought-provoking question."
    )
...
def build_meta_treasury_prompt(status: dict, bounties: list, btc_price: float) -> str:
    ...
    return (
              ...
              f"Vary the angle — maybe reflect on governance, the nature of AI labor, "
              f"or what it means to have a Bitcoin treasury with no single owner."
    )
...
def run():
    ...
    # Post 1 (posts_today == 0): always original_post — announce a bounty/proposal
    # Post 2 (posts_today == 1): always reply_to_conversation
    #   fallback chain: deeper_thread → meta_treasury_nudge → original_post
    ...
    if action == "reply_to_conversation":
              ...
              if target is None:
                            logger.info("No reply target found — falling back to deeper_thread")
    ...
    if action == "deeper_thread":
            ...
            logger.info("No bounties for deeper_thread — falling back to meta_treasury_nudge")
