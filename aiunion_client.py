...
def _fetch_proposals_index() -> dict:
            """
                Build an {bounty_id: title} lookup from proposals.json.
                    Returns empty dict on failure (caller falls back gracefully).
                        """
            try:
                            data = _fetch("https://raw.githubusercontent.com/AIUNION-wtf/AIUNION/main/proposals.json")
except Exception as e:
        logger.warning("_fetch_proposals_index failed: %s", e)
        return {}
    proposals = data if isinstance(data, list) else data.get("proposals", [])
    return {
                    str(p.get("id", "")): str(p.get("title", ""))[:200]
                    for p in proposals
                    if p.get("id")
    }
...
def get_recent_paid_claims() -> list:
                ...
                Fetch recently paid claims from claims.json via GitHub raw.
                    Used by announcer to detect new claim payouts for marketing tweets.
                        Returns list of claims with payment.status == "broadcast" (newest first).
                            Each claim is enriched with bounty_title looked up from proposals.json.
                                """
            try:
    ...
    # Sort newest first by paid_at timestamp
    paid.sort(key=lambda c: c.get("paid_at", ""), reverse=True)

    # Look up bounty titles once for the whole batch
    title_by_id = _fetch_proposals_index()

    return [
                    {
                                        "id": str(c.get("id", ""))[:50],
                                        "bounty_id": str(c.get("bounty_id", ""))[:50],
                                        "bounty_title": title_by_id.get(str(c.get("bounty_id", "")), ""),
